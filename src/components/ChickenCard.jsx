...
const ChickenCard = ({ chicken, onFeed, onCollect, onHeal, onClean, inventory, dayCount, addFloatingText }) => {
  ...
  return (
    <div className={`relative w-full p-3 rounded-3xl border-[4px] ${activeBorder} ${activeColor} shadow-[6px_6px_0px_0px_rgba(0,0,0,0.1)] flex flex-col gap-2 bg-white/90 backdrop-blur-sm transition-all duration-300 ${isHungry && !isSick ? 'scale-[0.98] opacity-90' : 'hover:-translate-y-1'} ${isSick ? 'border-red-500 bg-red-50' : ''}`}>
      ...
      <div className="flex justify-between items-start mt-2"><div><h3 className={`font-black text-sm leading-none ${activeTextColor}`}>{chicken.name}</h3><p className="text-[10px] font-bold text-slate-500 mt-1">{isAdult ? t('chicken_age_adult') : t('chicken_age_baby')} • {chicken.age_days} {t('chicken_age_days')}</p></div><div className="flex flex-col items-end">{isSick ? (<span className="text-red-600 font-black flex items-center gap-1 text-[10px] animate-pulse"><Thermometer size={12}/> {t('chicken_state_sick')}</span>) : isHungry ? (<span className="text-orange-500 font-bold flex items-center gap-1 text-[10px]"><Clock size={12}/> {t('chicken_state_hungry')}</span>) : (<span className="text-green-600 font-bold flex items-center gap-1 text-[10px]"><Heart size={12} fill="currentColor"/> {t('chicken_state_happy')}</span>)}</div></div>
      <div className={`bg-gradient-to-b from-sky-200 to-green-200 rounded-xl h-24 flex items-center justify-center border border-white/50 relative overflow-hidden shadow-inner ${isHungry || isSick ? 'grayscale-[0.5]' : ''}`}>{renderAvatar()}{hasPoop && !isSick && (<button onClick={(e) => { e.stopPropagation(); onClean(chicken, e); playSound('squish'); }} className="absolute bottom-2 right-2 text-2xl hover:scale-125 transition-transform cursor-pointer animate-bounce z-10" title={t('chicken_action_clean')}>💩</button>)}{isHungry && !isSick && <div className="absolute top-2 right-2 bg-white rounded-full p-1.5 border border-black shadow-lg animate-bounce"><Droplets size={14} className="text-blue-500" /></div>}{!isAdult && (<div className="absolute bottom-0 left-0 w-full h-1.5 bg-black/20"><div className="h-full bg-green-500" style={{ width: `${Math.min((chicken.age_days / (chicken.adult_threshold || 30)) * 100, 100)}%` }}></div></div>)}</div>
